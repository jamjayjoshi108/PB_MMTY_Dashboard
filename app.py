...
def load_data():
    ...
    # Loop through each vendor, download their sheet, and tag it
    for vendor_name, sheet_id in VENDOR_SHEETS.items():
        if sheet_id and sheet_id != "PASTE_..._SHEET_ID_HERE": # Skips if not filled yet
            csv_url = f"https://docs.google.com/spreadsheets/d/{sheet_id}/export?format=csv&gid=0"
            try:
                df = pd.read_csv(csv_url)
                df = df.dropna(subset=['Date', 'Yatri Name'], how='all')
                
                # Assign the vendor name to a new column for tracking!
    ...
    # Combine all 3 sheets into one massive master dataframe
    if all_dataframes:
        master_df = pd.concat(all_dataframes, ignore_index=True)
        master_df['Date'] = pd.to_datetime(master_df['Date'], format='%d/%m/%Y', errors='coerce')
        return master_df
    else:
        return pd.DataFrame() # Return empty if nothing loads
...
# -----------------------------------------------------------------------------
# 3. SIDEBAR FILTERS (Now with Vendor selection!)
# -----------------------------------------------------------------------------
...
if total_yatris > 0:
    ...
    with col2:
        ...
        fig_age = px.histogram(filtered_df, x="Age", nbins=10, 
                               color_discrete_sequence=['#1f77b4'], labels={'Age': 'Age Bracket'})
    ...
    # Reorder columns to show the Vendor first for better visibility
    cols = ['Vendor', 'Date', 'District', 'Halka', 'Block Name', 'Village Name', 'LGD Code', 'Booth No.', 'Guide Name', 'Guide Contact No.', 'Yatri Name', 'Gender', 'Age', 'Yatri Contact No.', 'Voter ID No.']
